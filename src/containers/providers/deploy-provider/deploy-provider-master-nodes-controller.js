angular.module('apf.containers.providersModule').controller('containers.deployProviderMasterNodesController',
  ...
  function ($rootScope, $scope, $timeout, $document, $resource) {
    ...
    var updateExistingVMs = function () {
      if ($scope.data.provisionOn === 'existingVms') {
        $resource('mock_data/containers/providers/nodes').get(function (data) {
    ...
    $scope.validateNodeCounts = function () {
      ...
      mastersValid = $scope.masterCountValid($scope.mastersCount);
      nodesValid = $scope.nodeCountValid($scope.nodesCount);
      storageNodesValid = $scope.storageNodeCountValid($scope.storageCount);
      dnsNodesValid = $scope.dnsNodeCountValid($scope.dnsCount);
      settingsValid = $scope.validateNodeSettings ();
    ...
    $scope.updateNodeSettings = function () {
      ...
      inputsValid = validString($scope.data.deploymentKey) && validString($scope.data.deploymentUsername);
