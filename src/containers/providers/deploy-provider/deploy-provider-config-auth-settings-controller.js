angular.module('apf.containers.providersModule').controller('containers.deployProviderAuthSettingsController',
  ['$rootScope', '$scope', '$timeout', '$document',
    function ($rootScope, $scope, $timeout, $document) {
      ...
      var validHtPassword = function () {
        ...
        invalid = $scope.data.authentication.htPassword.users.find(function (user) {
      ...
      $scope.onShow = function () {
        ...
        $scope.validateForm();
        $timeout(function () {
          var queryResult = $document[0].getElementById($scope.data.authentication.mode + '-initial-setting-input');
          if (queryResult) {
            queryResult.focus();
          }
        }, 200);
      ...
      $scope.browseCAFile = function () {
        var uploadfile = $document[0].getElementById($scope.data.authentication.mode + '-browse-ca-input');
